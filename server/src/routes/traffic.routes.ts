...
import {
  dailyUsageReportSchema,
  registerAppsSchema,
  totalUsageReportSchema,
} from './validation';
import logger from '../utils/logger';
import { extractDeviceTypeFromUserAgent } from '../utils/utils';
import { requireSubscription } from '../services/subscription.service';

const router = Router();

/**
 * POST /api/v1/traffic/register-apps
...
/**
 * POST /api/v1/traffic/per-process
 * Submit a per-process usage report from device
 */
router.post(
  '/per-process',
  requireDeviceAuth,
  async (req: Request, res: Response) => {
    try {
      ...
      // Check user's plan
      try {
        const { features } = await requireSubscription(device.userId);
        if (features.reportType !== 'per_process') {
          return res.status(403).json({
            success: false,
            message: 'plan_upgrade_required',
            error:
              'Per-process reporting is not available on your current plan. Please upgrade to access detailed per-application usage tracking.',
          });
        }
      } catch (error: unknown) {
        logger.error('Failed to check user plan features:', error);
...
/**
 * POST /api/v1/traffic/total-usage
 * Submit a total usage report from device
 */
router.post(
  '/total-usage',
  requireDeviceAuth,
  async (req: Request, res: Response) => {
    try {
      const parse = totalUsageReportSchema.safeParse(req.body);
      if (!parse.success) {
        return res.status(400).json({
          success: false,
          message: 'invalid payload',
      ...
      const TOTAL_USAGE_APP_IDENTIFIER = 'total-usage';
      const TOTAL_USAGE_APP_DISPLAY_NAME = 'Total Usage';

      const report = parse.data;

      const timestampDate = new Date(report.Timestamp);

      await deviceService.findOrCreateApp(
        device.id,
        TOTAL_USAGE_APP_IDENTIFIER,
        TOTAL_USAGE_APP_DISPLAY_NAME
      );
      ...
      await deviceService.createOrUpdateUsageReport({
        ...
        apps: [
          {
            identifier: TOTAL_USAGE_APP_IDENTIFIER,
      ...
      const totalRxMB = report.TotalRx / (1024 * 1024);
      const totalTxMB = report.TotalTx / (1024 * 1024);

      logger.debug(
        `Device: ${device.id}, Timestamp: ${timestampDate.toISOString()}, Total Usage - RX: ${totalRxMB.toFixed(2)} MB, TX: ${totalTxMB.toFixed(2)} MB`
      );

      return res.json({ success: true, message: 'received' });
    } catch (error: unknown) {
      logger.error('Total usage report error:', error);
      return res
        .status(500)
        .json({ success: false, message: 'internal server error' });
    }
