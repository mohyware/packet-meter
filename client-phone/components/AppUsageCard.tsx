  ...
  return (
    <ThemedView style={styles.appItem} lightColor="#f5f5f5" darkColor="#1f2123">
      <View style={styles.appHeader}>
        ...
        <ThemedText style={styles.totalUsage}>
      ...
      <View style={styles.usageDetails}>
        <View style={styles.usageRow}>
          <ThemedText style={styles.usageLabel}>
            Wi-Fi: ↓{formatBytes(item.wifi.rx)} ↑{formatBytes(item.wifi.tx)} ={' '}
            {formatBytes(item.wifi.total)}
          </ThemedText>
        </View>
        <View style={styles.usageRow}>
          <ThemedText style={styles.usageLabel}>
            Mobile: ↓{formatBytes(item.mobile.rx)} ↑
            {formatBytes(item.mobile.tx)} = {formatBytes(item.mobile.total)}
          </ThemedText>
        </View>
      </View>
    </ThemedView>
