...
namespace Daemon.Reporter
{
    public class Reporter
    {
        ...
        private async Task RegisterAppsRequest(List<AppRegistration> apps)
        {
            ...
            var jsonData = JsonSerializer.Serialize(appsPayload, PacketMeterJsonContext.Default.RegisterAppsRequest);
        ...
        {
            var jsonData = JsonSerializer.Serialize(usageReport, PacketMeterJsonContext.Default.UsageReportRequest);
            ...
            for (int attempt = 1; attempt <= _config.RetryAttempts; attempt++)
            {
                try
                {
                    ...
                    {
                        var responseContent = await response.Content.ReadAsStringAsync();
                        var serverResp = JsonSerializer.Deserialize(responseContent, PacketMeterJsonContext.Default.ServerResponse);
        ...
        {
            var jsonData = JsonSerializer.Serialize(totalUsageReport, PacketMeterJsonContext.Default.TotalUsageReportRequest);
