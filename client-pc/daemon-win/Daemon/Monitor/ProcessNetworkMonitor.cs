...
namespace Daemon.Monitor
{
    ...
    public class ProcessNetworkMonitor : IDisposable
    {
        ...
        public ProcessNetworkMonitor(Logger.Logger logger, MonitorConfig config)
        {
            _logger = logger;
            _config = config;
        }
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                lock (_sessionLock)
                {
                    ...
                    var sessionName = $"PacketMeterProcessMonitor_{Process.GetCurrentProcess().Id}";
        ...
        private void SaveUsageSnapshot()
        {
            try
            {
                ...
                var json = JsonSerializer.Serialize(snapshot, PacketMeterJsonContext.Default.PersistedUsage);
        ...
        private void LoadUsageSnapshot()
        {
            try
            {
                ...
                var data = JsonSerializer.Deserialize(json, PacketMeterJsonContext.Default.PersistedUsage);
        ...
        private void CleanupSessions()
        {
            try
            {
                ...
                {
                    if (!name.Contains("PacketMeter", StringComparison.OrdinalIgnoreCase))
                        continue;
